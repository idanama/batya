...
export default function Listing({ listing, modal }) {
  if (!listing) {
    listing = {
      for: 'sale',
  ...
  return (
    <Container maxW="6xl" pt={3} pb={3}>
      <Grid templateColumns="3fr 2fr" gap={4}>
        ...
        <Modal isOpen={photoModal} onClose={() => setPhotoModal(null)} size="full">
          <ModalOverlay />
          <ModalContent>
            <ModalCloseButton />
            <ModalBody h="100%" p="1rem" onClick={() => setPhotoModal(null)}>
              <Img src={photoModal} height="100%" width="100%" objectFit="contain" />
            </ModalBody>
          </ModalContent>
        </Modal>
        <Box>
